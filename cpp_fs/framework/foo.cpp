...
{
public:
    explicit FooProxy(int pipeFds[2]);
    virtual ~FooProxy();
...
void Constructor()
{
    ...
    f.FactoryRegister("foo", &FooCallable);
    ...
    boost::shared_ptr<Proxy> fooProxy(new FooProxy(pipeFds));
    
    f.InputMediatorRegister(fooProxy, pipeFds[0]);
    sleep(1);
...
FooProxy::FooProxy(int pipeFds[2])
: m_readFd(pipeFds[0])
, m_writeFd(pipeFds[1])
{
