...
enum NAMED_PIPE_PING_RETURN_VAL
{
    FUNC_RETURN_VAL_ERROR = -1,
    NAMED_PIPE_PING_SUCCESS = 0,
    NAMED_PIPE_PING_MKFIFO_ERROR,
    NAMED_PIPE_PING_OPEN_ERROR
};
...
int main()
{
    ...
    int fifo_fd = 0;
    int status = NAMED_PIPE_PING_SUCCESS;
    int i = 0;
    char buff_read[1024] = {0};
    char buff_write[] = "Ping!";

    status = mkfifo(fifo_file_path, 0666);
    if(FUNC_RETURN_VAL_ERROR == status)
    {
        perror("mkfifo() failed.\n");

        return NAMED_PIPE_PING_MKFIFO_ERROR;
    }
    ...
    for(i = 0; NUM_OF_LOOPS > i; ++i)
    {
        ...
        if(FUNC_RETURN_VAL_ERROR == fifo_fd)
        {
            ...
            return NAMED_PIPE_PING_OPEN_ERROR;
        }
    ...
    return NAMED_PIPE_PING_SUCCESS;
}